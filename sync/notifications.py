...
def init_notification_system():
    """
    Инициализирует систему уведомлений в главном Qt потоке
    ДОЛЖНА быть вызвана после создания QApplication!
    """
    global _manager
    if _manager is not None:
        logger.warning("NotificationManager already initialized")
        return _manager
    ...
    with _manager_lock:
        if _manager is None:
            ...
            if app is None:
                raise RuntimeError(
                    "init_notification_system() must be called after QApplication initialization"
                )
            
            _manager = NotificationManager()
            logger.info("✓ NotificationManager initialized in main thread")
    
    return _manager


def _get_manager():
    """Получить глобальный экземпляр менеджера (lazy initialization)"""
    global _manager
    if _manager is None:
        logger.warning(
            "NotificationManager was not explicitly initialized. "
            "Attempting lazy initialization (may cause threading issues)"
        )
        return init_notification_system()
